...
const log = require('debug')('forms:refs')
...
jrefs.replace = async function(json, ids, root) {
  ...
  for (k in json) {
    ...
    if ((v != null) && (v[jrefs.reftoken] != null)) {
      ...
      if (Array.isArray(ref)) {
        ...
        json[k] = ids[ref]
      } else if ( String(ref).match(/^https?:/)) {
        url = ref.match(/^[^#]*/);
        if (!jrefs.cache[url]) {
          //log('url', url);
          //const opts = { "withCredentials": true, "crossdomain": true };
          //const data = await axios.get(url[0], opts).then(resp => resp.data).catch(console.error);
          //log('json', data);
          //jrefs.cache[url] = jrefs.resolve(data);
          const opts = {
            headers: {
              'Accept': 'application/json, text/plain, */*'
            }
          };
          const jsonString = request("GET", url[0], opts).getBody().toString();
          //log('json', jsonString);
          jrefs.cache[url] = jrefs.resolve(JSON.parse(jsonString));

        }
        json[k] = jrefs.cache[url];
        if (ref.match(jrefs.pathtoken)) {
          jsonpointer = ref.replace(new RegExp(".*" + jrefs.pathtoken), jrefs.pathtoken);
      ...
      if ((((ref1 = json[k]) != null ? ref1.length : void 0) != null) && ((ref2 = json[k]) != null ? ref2.length : void 0) === 0 && jrefs.debug) {
        results.push(log(ref + ' reference not found'))
      } else {
        results.push(void 0)
      }
    } else {
